...
// Two levels: this slot is `hosts/web/` since the platform extraction freed that
// name (§16.4). It used to be `frontend/`, one level down, and the freshness
// plugin silently checked the wrong tree for a while after the move — it
// reported src/gen and src/wasm MISSING while both were sitting right there.
