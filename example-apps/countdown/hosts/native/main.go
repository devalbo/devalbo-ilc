// countdown — native CLI host (the native tier slot, Decision 34).
//
...
import (
	...
	_ "github.com/you/countdown/engine" // importing the engine registers its commands
...
func main() {
	...
	//
	// `./.countdown/`: project-local like git, so running in two projects keeps two
	...
	if err := platform.Boot(platform.BootOptions{
		...
		os.Stderr.WriteString("countdown: " + err.Error() + "\n")
		os.Exit(2)
	}
...
func app(port platform.EnginePort, stdout, stderr io.Writer, stdin io.Reader) cli.App {
	return cli.App{
		Name:  "countdown",
		Short: "countdown — one engine, a terminal and a browser",
		...
		Commands: append(append([]clispec.Command{}, countdownv1.AppServiceCLI...), ilcv1.PlatformServiceCLI...),
		...
		Render: map[uint32]cli.Renderer{
			// PRINTS NOTHING, on purpose — the engine already streamed this text
			// to stdout, and rendering it here too shows it twice.
			//
			// The division: the ENGINE prints prose, for tiers that cannot decode
			// a typed response (a badge has no schema and never will). A HOST
			// renders STRUCTURE, for tiers that can — a browser building a DOM, a
			// script wanting a field. A greet has no structure worth adding, so
			// the honest renderer is an empty one.
			//
			// When your command returns something a terminal can present better
			// than prose — a table, a count, an exit condition — render THAT
			// here, not a second copy of the text.
			// The ticks are PRINTED by the engine as they happen; this renders
			// structure, and a countdown's structure is the one word it ends on.
			// Returning nil keeps the terminal from repeating it.
			countdownv1.MethodCount: render(func(_ io.Writer, _ *countdownv1.CountResponse) error {
				return nil
			}),
