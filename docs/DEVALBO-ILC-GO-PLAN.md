...
### 6.4 Display — THREE render paths, and the app author picks (output-only)

**Display is optional** (Decision 34). Getting something on a screen does not require this capability at
all; the third path below needs no capability, no new schema, and no new WIT.

> **Refined by Decision 35 (2026-07-29).** The first two paths are sketched below as an **import** the engine
> pushes. They should be a **pulled export** instead — `render(state) -> draw-list` as one of the app's own
> commands — because **timing belongs to the tier**: only the host knows its refresh cost, its RAM budget and
> what is already on screen. Pulling also removes the obligation WIT's lack of optional imports creates, so a
> screenless tier owes no stub. The table's *reasoning* is unchanged; its mechanism is.

| Path | The app emits | Who owns presentation | What it costs |
| --- | --- | --- | --- |
| **Draw-command list** (`draw`) | high-level ops (rect/text/image) as protobuf; host rasterizes. ESP32-S3 TFT → driver calls; React → canvas | app | the Display capability; the app branches on the manifest per tier |
| **Retained widget tree** (`render`) | VDOM-like tree as protobuf; host diffs + renders. React → elements; a widget-capable MCU host → LVGL-style | app structure, host look | the Display capability + a widget vocabulary both ends agree on |
| **Semantic events** (Decision 34) | *what is true* — `board`, `turn`, `winner` — over Events (§6.3) | **host, entirely** | one tier slot per tier, written per app. No capability |

For the first two, resolution, colour format, and which paths the host supports arrive in the
**environment manifest** (§6.4a, Decision 32) rather than through a `describe()` import — so **one handler
targets a 320×240 TFT and a browser canvas from the same logic** by branching on the reported caps, and a
browser resize re-sends the manifest instead of requiring a poll.

**How to choose.** The first two put presentation in the app: write it once, and every tier gets it — right
when an app has an open-ended, data-shaped UI (notes' record list) and wrong when the tiers have nothing
visually in common. **Decision 35 sharpens this test:** dissimilarity of output is the weaker criterion, because
two tiers can share a screen and still need different rendering — one may be running an interpreter. The
question is who knows the cost of a repaint, and that is always the tier. The third inverts it: a tic-tac-toe board as DOM, as a TFT grid, and as terminal ASCII
share no structure, but the payload — whose turn, what is on the board — is identical everywhere and tiny,
so the app stops shipping presentation entirely and each host draws what it likes.

The choice is **per event, not per app**, and mixing is normal: `ilc.data-changed` is pure invalidation,
`notes.record-changed` is semantic but its host just re-lists, `game.state-changed` is semantic and
rendered. It is also free to defer, because a host that does not recognize a topic ignores it and the app
cannot tell (Decision 33's D4) — so an app emits semantic events unconditionally and gains a native
renderer later without changing a line of engine code.

**The cost, stated plainly:** the third path moves per-tier variation *out* of the app and *into* the
hosts, which is the one place this architecture permits tiers to differ. `hosts/<tier>/` code renders and
never decides — see Decision 34 and `HOST-LAYER-PLAN.md` for why parity cannot catch a violation and what
does.
...
> **BUILT (2026-07-28)** — [`ENVIRONMENT-PLAN.md`](./ENVIRONMENT-PLAN.md) has the design decisions, the
> phases and what each falsification actually showed. What shipped differs from the sketch below in three
> ways worth knowing before reading on:
>
> - **Registration follows the manifest.** `SetEnvironment` does not merely record facts; it is the trigger
>   for registering capability-dependent verbs (`RegisterDiscovered`). That makes "the host sends it before
>   anything else" a correctness requirement rather than a convention, and `platform.Boot` owns the order.
> - **A revision is required and non-zero.** An unchanged revision is a deliberate no-op, because
>   re-applying would rebuild the command surface underneath a host that only repeated itself.
> - **The surface is introspectable** (`GetCommandSurface`) and is a parity vector. Parity compares
>   results, so registration was invisible to it — twice during the build, parity was green while every
>   filesystem verb on both tiers was broken.
>
> Display facts are **not** in the shipped message: Decision 34 made Display optional, and a field for an
> unbuilt capability is a branch nothing tests. Only the filesystem is described so far; the index lands
> with the index.
