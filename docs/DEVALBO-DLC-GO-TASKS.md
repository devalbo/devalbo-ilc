...
- [x] `open` / `rebuild-index` — `open` reads the record's own file (never the index); `rebuild-index` is inherited from the platform. `update` is still open, and is the first command that can make a projection go stale between writes
