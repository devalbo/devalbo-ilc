...
// WHICH BUILD OF THE FIRMWARE THIS IS, generated by build.rs. The doc comment
// lives on the constant inside — `include!` cannot carry one.
...
fn phase_instance_starting<U: core::fmt::Write>(
    ...
    // built to make impossible.
    {
        ...
        // WHICH BUILD OF THE APP THIS IS, asked once while the instance is being
        // opened.
        //
        // THE WORLD SHOULD ALWAYS KNOW WHAT IS RUNNING. It already discovers
        // WHICH app — a loader runs payloads it was never built for — and the
        // version is the half that makes a report reproducible: two builds of
        // one app are the same catalog entry, the same size band, and different
        // software.
        //
        // ONCE, NOT PER REQUEST: an app's version is compiled in and cannot
        // change while an instance lives, and `GetWorldState` is answered from
        // an interrupt that must not enter the store.
        //
        // `Version` is method 1, inherited by every app through `RegisterAll` —
        // so this works for an app that has never heard of this firmware.
        if let Ok(reply) = host.execute(manifest::METHOD_ID_VERSION, &[]) {
            if reply.success {
                if let Some(version) = dlc_platform_embedded::spec::first_string(
                    &reply.output,
                    dlc_platform_embedded::control::platform::fields::F_VERSION_RESPONSE_VERSION,
                ) {
                    dlc_platform_embedded::appversion::set(version.as_bytes());
                    report.note(format_args!("app: {} {}", selected.name, version));
                }
            }
        }
    ...
    #[cfg(not(badge_input_off))]
    {
        ...
        for flag in flags.iter().take(count) {
            ...
            match flag.kind {
                ...
                kind if kind == spec::SPEC_KIND_STRING => {
                ...
                kind if spec::is_integer(kind) || kind == spec::SPEC_KIND_BOOL => {
                    let prompt = spec::help_of(answer, flag)
                        .or_else(|| spec::name_of(answer, flag))
                        .unwrap_or("value?");
                    let start = spec::default_number(answer, flag);
                    let shape = if kind == spec::SPEC_KIND_BOOL {
                ...
                kind if kind == spec::SPEC_KIND_ENUM => {
                    let prompt = spec::help_of(answer, flag)
                        .or_else(|| spec::name_of(answer, flag))
                        .unwrap_or("choose?");
...
fn phase_instance_stopping<U: core::fmt::Write>(
    ...
    dlc_platform_embedded::activity::clear();
    // AND THE VERSION GOES WITH IT. A world reporting the previous app's version
    // is worse than one reporting none: empty says "nothing running", stale says
    // something false about what is.
    dlc_platform_embedded::appversion::clear();
