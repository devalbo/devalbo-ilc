// Spike 3 vendored copy of preview2-shim@0.19.0 browser filesystem.
// Upstream now coerces BigInt read bounds and compares write offset to 0n.
// Still patched: getFlags/setSize stubs, openAt truncate, write offset coercion
// when callers pass Number(0) instead of 0n.
import { inputStreamCreate, outputStreamCreate } from "@bytecodealliance/preview2-shim/io";
import { environment } from "@bytecodealliance/preview2-shim/environment";
import { _setCwd, _getCwd } from "@bytecodealliance/preview2-shim/config";
export { _setCwd } from "@bytecodealliance/preview2-shim/config";
...
// PATCH (devalbo-ilc): resolve a path to its parent directory entry and leaf
// name, so removal can delete the key. getChildEntry() returns the entry itself,
// which is not enough to unlink it from its parent.
function resolveParent(rootEntry, path) {
...
class Descriptor {
    ...
    getFlags() {
        // Upstream returns {} (all false) → TinyGo treats the FD as non-writable.
        if (this.#entry?.dir) {
            return {
                read: true,
                write: true,
    ...
    setSize(size) {
        // Upstream stub is a no-op; TinyGo WriteFile truncates via setSize(0).
        if (this.#entry?.dir) {
            throw "is-directory";
        }
        const n = coerceToSafeIntegerNumber(size);
        const src = getSource(this.#entry);
        if (n === src.byteLength) {
            return;
        }
        if (n < src.byteLength) {
            this.#entry.source = src.slice(0, n);
            return;
        }
        const next = new Uint8Array(n);
        next.set(src);
        this.#entry.source = next;
    }
    ...
    write(buffer, offset) {
        // Upstream uses `offset !== 0n`, which still throws if a Number(0) sneaks in.
        const off = coerceToSafeIntegerNumber(offset);
        if (off !== 0) {
            throw "invalid-seek";
        }
    ...
    openAt(_pathFlags, path, openFlags, _flags) {
        ...
        if (openFlags?.truncate && childEntry.source) {
    ...
    removeDirectoryAt(path) {
        // PATCH (devalbo-ilc): upstream logs and returns, so a delete silently
        // succeeds while the tree keeps the entry — reset-fs / import --replace
        // would report success and change nothing. Actually unlink it.
        const { parent, name } = resolveParent(this.#entry, path);
    ...
    unlinkFileAt(path) {
        // PATCH (devalbo-ilc): see removeDirectoryAt — upstream is a no-op.
        const { parent, name } = resolveParent(this.#entry, path);
        const entry = parent.dir[name];
        if (!entry) {
            throw "no-entry";
